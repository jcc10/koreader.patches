--[[
    Forked from https://github.com/joshuacant/KOReader.patches/blob/main/2-reader-header-centered.lua
    Added truncation function to display (Tuned for Kindle 4th Gen display)
...
ReaderView.paintTo = function(self, bb, x, y)
    ...
    local trunc_author = string.sub(book_author, 1, 15)
    local trunc_title = string.sub(book_title, 1, 25)
    local full_header = string.format("%s %s %s", trunc_author, separator.en_dash, trunc_title)
    local centered_header = string.sub(full_header, 1, 34)
    if full_header ~= centered_header then
        centered_header = string.format("%s...", centered_header)
    end
