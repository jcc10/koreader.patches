--[[
    Forked from https://github.com/joshuacant/KOReader.patches/blob/main/2-reader-header-cornered.lua
    This adds a truncation code to prevent overlap.
    It additionally adds the ability to check WiFi status and if the SSH server is running, to prevent leaving them on.
...
ReaderView.paintTo = function(self, bb, x, y)
    ...
    local left_corner_header = string.format("%s%s%s", wifi_status, ssh_status, battery)

    local right_corner_full = string.format("%s", book_title)
    local right_corner_header = string.sub(right_corner_full, 1, 20)
